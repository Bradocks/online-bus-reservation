...
// Fetch all users
$sql = "SELECT id, `name`, mobile_number, email, `role`, `user_name`, `password`, id_no, dob, gender, staff_id FROM user WHERE `role` = 'driver'";
...
<div class="container mt-4">
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="/admin/manage_drivers.php?edit=<?php echo $user_to_update['id']; ?>">
                    ...
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            echo "<tr>
                                    ...
                                    <td>{$row['staff_id']}</td>
                                    <td>
                                        <a href='/admin/manage_drivers.php?edit={$row['id']}' class='btn btn-warning btn-sm'>Edit</a>
                                    </td>
                                    <td>
                        ...
                        echo "<tr><td colspan='11'>No drivers available</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
...
<script>
    function confirmDelete(id) {
        if (confirm("Are you sure you want to delete this user?")) {
            window.location.href = '/admin/manage_drivers.php?delete=' + id;
        }
    }
</script>
